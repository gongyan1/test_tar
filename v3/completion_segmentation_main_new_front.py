...
args.result = os.path.join('..', 'results/v3')
...
def main():
    ...
    for epoch in range(args.start_epoch, args.epochs):
        ...
        _, _, train_lane_loss, train_lane_acc, train_total_acc = iterate("train", args, train_loader, model, optimizer, logger, best_acc, epoch)  # train for one epoch
    ...
    #plot.plot('lane_val', args.epochs, lane_acc_list = val_lane_acc_list)
        ...
        with open('log/train_lane_loss_v3_'+time.strftime('%Y-%m-%d-%H:%M:%S',time.localtime(time.time()))+'.txt', 'w+') as f:
            train_lane_loss_list_w = [str(line) for line in train_lane_loss_list]
            f.writelines('\n'.join(train_lane_loss_list_w))
        with open('log/train_lane_acc_v3_'+time.strftime('%Y-%m-%d-%H:%M:%S',time.localtime(time.time()))+'.txt', 'w+') as f:
            train_lane_acc_list_w = [str(line) for line in train_lane_acc_list]
            f.writelines('\n'.join(train_lane_acc_list_w))
        with open('log/train_total_acc_v3_'+time.strftime('%Y-%m-%d-%H:%M:%S',time.localtime(time.time()))+'.txt', 'w+') as f:
            train_total_acc_list_w = [str(line) for line in train_total_acc_list]
            f.writelines('\n'.join(train_total_acc_list_w))
        with open('log/val_lane_acc_v3_'+time.strftime('%Y-%m-%d-%H:%M:%S',time.localtime(time.time()))+'.txt', 'w+') as f:
            val_lane_acc_list_w = [str(line) for line in val_lane_acc_list]
            f.writelines('\n'.join(val_lane_acc_list_w))
        with open('log/val_total_acc_v3_'+time.strftime('%Y-%m-%d-%H:%M:%S',time.localtime(time.time()))+'.txt', 'w+') as f:
